...
import cz.mendelu.tomas.graphpef.database.DBContractNames.CategoryTable;
import cz.mendelu.tomas.graphpef.database.DBContractNames.QuestionsTable;
import cz.mendelu.tomas.graphpef.database.DBContractNames.QuizAnswerTable;
import cz.mendelu.tomas.graphpef.helperObjects.QuizQuestion;

import static android.database.Cursor.FIELD_TYPE_NULL;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_ANSWER1;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_ANSWER2;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_ANSWER3;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_ANSWER4;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_CATEGORY;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_CATEGORY_ID;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_CORRECT_ANSWER;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_SUBJECT;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_QUESTION_TEXT;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_USER_ANSWERS_STREAK;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_USER_MAIL;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_USER_POINTS;
import static cz.mendelu.tomas.graphpef.database.DBContractNames.FIRESTORE_USER_POINTS_STREAK;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private CollectionReference questionsRef = onlineDatabase.collection(DBContractNames.COLLECTION_NAME_QUIZ_QUESTIONS);
    private CollectionReference categoriesRef = onlineDatabase.collection(DBContractNames.COLLECTION_NAME_QUIZ_CATEGORIES);
    private DocumentReference usersRef;
    private DocumentReference databaseVersion;
    private CollectionReference answeredQuestions = onlineDatabase.collection(DBContractNames.COLLECTION_NAME_ANSWERED_QUESTIONS);
    ...
    public DBHelper(@Nullable Context context) {
        super(context, DBContractNames.DATABASE_NAME, null, DBContractNames.DATABASE_VERSION);
    }
    ...
    private void getQuizCategoriesFromFirestore() {
        categoriesRef.get()
                ...
                }).addOnFailureListener((@NonNull Exception e) -> Log.e(TAG, e.getMessage()));
    ...
    private void fillCategoriesTable(List<DocumentSnapshot> documents) {
        ...
        for (DocumentSnapshot document : documents) {
            ...
            values.add(document.getString(DBContractNames.FIRESTORE_CATEGORIES_TITLE));
            values.add(document.getId());
            if (document.get(DBContractNames.FIRESTORE_CATEGORIES_UNLOCKED) != null) {
                values.add("0");
            }
    ...
    private void fillQuestionsTable(List<DocumentSnapshot> documents) {
        ...
        for (DocumentSnapshot document : documents) {
            try {
                Double correctAnswer = document.getDouble(FIRESTORE_QUESTION_CORRECT_ANSWER);
                int correctAnswerInt = 0;
                if (correctAnswer != null) {
                    correctAnswerInt = correctAnswer.intValue();
                }
                ...
                Log.e(TAG, "FIRESTORE_QUESTION_CORRECT_ANSWER is null [" + e.getMessage() + "]");
            }
    ...
    private void addQuestion(QuizQuestion quizQuestion) {
        ...
        values.put(QuestionsTable.COLUMN_CATEGORY_ID, quizQuestion.getCategoryId());
    ...
    public List<QuizQuestion> getUnlockedQuestions() {
        Log.d(TAG, "getUnlockedQuestions: ");

        List<List<String>> unlockedCategories = getAllUnlockedCategories();

        List<QuizQuestion> quizQuestions = new ArrayList<>();
        db = getReadableDatabase();
        for (List<String> category : unlockedCategories) {
            Log.d(TAG, "getUnlockedQuestions " + "SELECT * FROM " + QuestionsTable.QUIZ_TABLE_NAME + " WHERE " + QuestionsTable.COLUMN_CATEGORY_ID + " = '" + category.get(2)/*FirestoreId*/ + "'");
            Cursor c = db.rawQuery("SELECT * FROM " + QuestionsTable.QUIZ_TABLE_NAME + " WHERE " + QuestionsTable.COLUMN_CATEGORY_ID + " = '" + category.get(2)/*FirestoreId*/ + "'", null);
    ...
    public List<QuizQuestion> getQuestionsCategory(String categoryId) {
        Log.d(TAG, "getAllQuestions: ");
        List<QuizQuestion> quizQuestions = new ArrayList<>();
        db = getReadableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + QuestionsTable.QUIZ_TABLE_NAME + " WHERE " + QuestionsTable.COLUMN_CATEGORY_ID + " = " + categoryId, null);
    ...
    private void addQuestionToFirestore(Map<String, Object> newQuestion) {
        questionsRef
                .add(newQuestion)
                .addOnSuccessListener((DocumentReference documentReference) -> Log.d(TAG, "addQuestionToFirestore - Success"))
                .addOnFailureListener((@NonNull Exception e) -> Log.e(TAG, e.getMessage()));
    }
    ...
    private void addQuestionAnswered(String fireStoreID, boolean saveToFirestore) {
        Log.d(TAG, "addQuestionAnswered: questionID[" + fireStoreID + "]");

        if (saveToFirestore) {
            Map<String, Object> answeredQuestion = new HashMap<>();
            answeredQuestion.put(DBContractNames.FIRESTORE_ANSWERED_QUESTIONS_QUESTION_ID, fireStoreID);
            FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
            if (user != null)
                answeredQuestion.put(DBContractNames.FIRESTORE_ANSWERED_QUESTIONS_USER_ID, user.getUid());
            else
                Log.e(TAG, "FireBase user == null");
            answeredQuestion.put(DBContractNames.FIRESTORE_ANSWERED_QUESTIONS_TIMETAG, getTimeNow());
        ...
        db.update(QuestionsTable.QUIZ_TABLE_NAME, updatedValue, QuestionsTable.COLUMN_FIRESTORE_ID + " = '" + fireStoreID + "'", null);
    }

    public void addQuestionAnswered(String fireStoreID) {
        addQuestionAnswered(fireStoreID, true);
    }
    ...
    //returns (name,price,id)
    public ArrayList<List<String>> getAllUnlockedCategories() {
        Log.d(TAG, "getAllUnlockedCategories");
        ArrayList<List<String>> list = new ArrayList<>();
        db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + CategoryTable.CATEGORY_TABLE_NAME, null);
        ...
        if (cursor.moveToFirst()) {
            do {
                boolean unLocked = cursor.getType(cursor.getColumnIndex(CategoryTable.COLUMN_UNLOCKED)) != FIELD_TYPE_NULL;
                String categoryName = cursor.getString(cursor.getColumnIndex(CategoryTable.COLUMN_CATEGORY));
                if (unLocked) {
                    Integer price = (list.size() + 1) * 75;
                    Log.d(TAG, "category [" + categoryName + "] added with price[" + price + "]");
    ...
    public int getNumUnlockedQuestions() {
        ...
        for (List<String> listEntry : getAllUnlockedCategories()) {
    ...
    public void createUserRef(String userID, String mail) {
        ...
        usersRef = onlineDatabase.collection(DBContractNames.COLLECTION_NAME_USERS).document(userID);
        usersRef.get().addOnSuccessListener(documentSnapshot -> {
            ...
            if (documentSnapshot.exists()) {
                ...
                newUser.put(DBContractNames.FIRESTORE_USER_ID, userID);
                newUser.put(FIRESTORE_USER_POINTS, 0);
                newUser.put(FIRESTORE_USER_ANSWERS_STREAK, 0);
                newUser.put(FIRESTORE_USER_POINTS_STREAK, 0);
                //                    onlineDatabase.collection(DBContractNames.COLLECTION_NAME_USERS).add(newUser)
                onlineDatabase.collection(DBContractNames.COLLECTION_NAME_USERS).document(userID).set(newUser).addOnSuccessListener(aVoid -> {
    ...
    //check data saved on Firestore
    private void initialUserDataRead(DocumentSnapshot documentSnapshot) {
        Log.d(TAG, "initialUserDataRead");
        Double userPointsDouble = documentSnapshot.getDouble(FIRESTORE_USER_POINTS);
        Double userHighScorePointsDouble = documentSnapshot.getDouble(FIRESTORE_USER_POINTS_STREAK);
        Double userHighScoreAnswersDouble = documentSnapshot.getDouble(FIRESTORE_USER_ANSWERS_STREAK);

        if (userPointsDouble == null || userHighScorePointsDouble == null || userHighScoreAnswersDouble == null) {
            Log.e(TAG, "User value is null");
            return;
        } else {
            int userPoints = userPointsDouble.intValue();
            int userHighScorePoints = userHighScorePointsDouble.intValue();
            int userHighScoreAnswers = userHighScoreAnswersDouble.intValue();
        ...
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        if (user != null) {
            answeredQuestions.whereEqualTo(DBContractNames.FIRESTORE_ANSWERED_QUESTIONS_USER_ID, user.getUid())
                    .get().addOnSuccessListener(queryDocumentSnapshots -> {
                if (queryDocumentSnapshots.isEmpty()) {
                    Log.d(TAG, "answeredQuestions for this user is empty");
                } else {
                    updateQuestionsAnswered(queryDocumentSnapshots.getDocuments());
                    observable.updateUI();
                }
            }).addOnFailureListener((@NonNull Exception e) -> Log.e(TAG, e.getMessage()));
        } else {
    ...
    //helper method to update already ansered questions after relog from firestore
    private void updateQuestionsAnswered(List<DocumentSnapshot> documentSnapshots) {
        Log.d(TAG, "updateQuestionsAnswered documentSnapshots size[" + documentSnapshots.size() + "]");
        for (DocumentSnapshot doc : documentSnapshots) {
            addQuestionAnswered(doc.getString(DBContractNames.FIRESTORE_ANSWERED_QUESTIONS_QUESTION_ID), false /*saveToFirestore*/);
        }
    }

    //After change of score and streaks update data on firestore
    public void updateUserStats() {
        ...
        usersRef.update(updatedUser)
                .addOnSuccessListener((Void aVoid) -> Log.i(TAG, "updateUserStats User stats updated"))
                .addOnFailureListener((@NonNull Exception e) -> Log.e(TAG, e.getMessage()));
    }
    ...
    //workaround for async update of mainscreen(scor + categories)
